...
Typed_Array_Container :: struct($T: typeid) {
	...
	storage:   internals.nTyped_Array_Storage(T),
}

create_typed_array_container :: proc(
	$T: typeid,
	min_size := internals.MIN_ELEMENT_NUMBER,
...
insert_into_typed_array_container :: proc(
    ...
    handle := internals.allocate_typed_handle(&container.allocator)
    internals.insert_into_typed_array_storage(&container.storage, handle, value)
    return handle
}
